...
# Modify index.py to make a program indexbykeyword.py that takes a
# file name from the command line and makes an index from standard input using
# only the keywords in that file. Note: Using the same file for indexing and keywords
# should give the same result as index.py.

minLength = int(sys.argv[1])
minCount = int(sys.argv[2])

instream_1 = InStream('text1.txt')
instream_2 = InStream('text2.txt')
text1 = instream_1.readAll()
text2 = instream_2.readAll()

# words = stdio.readAllStrings()
outstream = OutStream('compare1and2.txt')
...
for word in bst:
    ...
    if word in bst2:
        ...
        for occurrence in occurrences:
            out_text += (str(occurrence) + " ")
            outstream.write(out_text)
